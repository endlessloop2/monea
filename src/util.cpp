...
const char * const BITCOIN_CONF_FILENAME = "monea.conf";
const char * const BITCOIN_PID_FILENAME = "monead.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "monea" is a composite category enabling all Monea-related debug output
            if(ptrCategory->count(string("monea"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MoneaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MoneaCore
    // Mac: ~/Library/Application Support/MoneaCore
    // Unix: ~/.moneacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MoneaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MoneaCore";
#else
    // Unix
    return pathRet / ".moneacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty monea.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override monea.conf
