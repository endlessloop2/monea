...
bool CheckProofOfWork(const CBlockHeader& block, const Consensus::Params& params)
{
    ...
    if (!CheckProofOfWork(block.GetPoWHash(), block.nBits, params))
        return error("%s (val.cpp): proof of work failed", __func__);
    */
...
*/
/*CAmount GetBlockSubsidy_OLD(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    bool fDIP0001Active_context = false; //disable DIP0001
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("%s: new best=%s height=%d algo=%d (%s) nVersion=0x%08x version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utx)", __func__,
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), chainActive.Tip()->GetAlgo(),
      GetAlgoName(chainActive.Tip()->GetAlgo(), chainActive.Tip()->GetBlockTime()),chainActive.Tip()->nVersion, chainActive.Tip()->GetBaseVersion(),
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != NULL; i++)
        {
            ...
            if (pindex->GetBaseVersion() > VERSIONBITS_LAST_OLD_BLOCK_VERSION && (pindex->GetBaseVersion() & ~nExpectedVersion) != 0) // GetBaseVersion() removes the algo from nVersion
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (fixedVersion < 2 && IsSuperMajority(2, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams))
        return state.Invalid(error("%s: rejected nVersion=1 block", __func__),
                             REJECT_OBSOLETE, "bad-version");

    // Reject block.nVersion=2 blocks when 95% (75% on testnet) of the network has upgraded:
    if (fixedVersion < 3 && IsSuperMajority(3, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams))
        return state.Invalid(error("%s: rejected nVersion=2 block", __func__),
                             REJECT_OBSOLETE, "bad-version");

    // Reject block.nVersion=3 blocks when 95% (75% on testnet) of the network has upgraded:
    if (fixedVersion < 4 && IsSuperMajority(4, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams))
