...
UniValue generate(const UniValue& params, bool fHelp)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        while (!CheckProofOfWork(pblock->GetPoWHash(pblock->GetAlgo()), pblock->GetAlgo(), pblock->nBits, Params().GetConsensus())) {
...
UniValue getmininginfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"difficulty_scrypt\": xxx.xxxxx (numeric) The current difficulty for scrypt\n"
            ...
            "  \"difficulty_yescrypt\": xxx.xxxxx (numeric) The current difficulty for yescrypt\n"
    ...
    obj.push_back(Pair("difficulty_scrypt", (double)GetDifficulty(NULL, ALGO_SLOT2)));
    obj.push_back(Pair("difficulty_neoscrypt", (double)GetDifficulty(NULL, ALGO_SLOT3)));
    obj.push_back(Pair("difficulty_argon2d", (double)GetDifficulty(NULL, ALGO_SLOT4)));
    obj.push_back(Pair("difficulty_yescrypt", (double)GetDifficulty(NULL, ALGO_SLOT5)));
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Monea Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Monea Core is downloading blocks...");
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Monea Core is downloading masternode winners...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        && !masternodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Monea Core is syncing with network...");
